export default function Teconologias(){
    return(
        <>
        {/* <a className=""> */}
        <div id="tecnologias" className="h-screen-1 pt-14 mb-72 bg-red-500">
        <div className="flex flex-nowrap h-full">
                <div className="w-1/6 ">
                    
                </div>
                <div className="w-5/6  h-full">
                    <div className="text-center">
                        <span className="md:text-4xl text-2xl font-bold font-serif  text-white">Tecnologías</span>
                    </div>
                    <div>
                        <p className="text-white text-base md:text-lg">Algunas de las tecnologías que manejo y utilizo para crear aplicaciones:</p>
                    </div>
                    <div className="md:flex md:flex-nowrap mt-4  h-auto w-full ">
                        {/* className="flex flex-nowrap md:flex-none w-full */}
                        <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2 bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/html.2ba4fabc69a89a8f71e6.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>HTML</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2 bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/css.69a82c2d9e45c933a9cb.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>CSS</p>
                            </div>
                        </div>
                        </div>
                        {/* className="flex flex-nowrap md:flex-none w-full" */}
                        <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/javascript.1ccd6ef9bb1f9c84ef00.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>JavaScript</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/react.0cf951a69d8e58f83f9d.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>React</p>
                            </div>
                        </div>
                        </div>
                    </div>
                    <div className="md:flex md:flex-nowrap   h-auto w-full">
                    <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT4rwMSTN2sKZQivG64hnJH1C3dWwJ3MwT_Rg&usqp=CAU" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>Redux</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2 bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/node.952a9ea986dcfa5229ad.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>Node.js</p>
                            </div>
                        </div>
                        </div>
                        <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2 bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="data:image/png;base64," className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>Sequelize</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/postgresql.23588a8822f8b0188565.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>PostgreSQL</p>
                            </div>
                        </div>
                        </div>
                    </div>
                    <div className="md:flex md:flex-nowrap   h-auto w-full">
                        <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/git-icon.db538452a3fb6ffbc088.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>Git</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://miro.medium.com/max/1400/1*scZ5_B1mKwZxqP0KuCkf_w.png" className="w-32 h-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>Node.js Express</p>
                            </div>
                        </div >
                        </div >
                        <div className="flex flex-nowrap md:w-1/2">

                        </div>
                    </div>
                    <div>
                        <p className="text-white md:text-lg text-base pt-5">Actualmente estoy aprendiendo nuevas tecnologías como:</p>
                    </div>
                    <div className="md:flex md:flex-nowrap   h-auto w-full">
                    <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2   bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzPRJtaaZTTVlTrHBLDAvu3ybCogGxDgjBIRmWOH7s8Q&s" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>TypeScript</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2  bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/react.0cf951a69d8e58f83f9d.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>React Native</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex flex-nowrap md:w-1/2">
                        <div className="md:w-full w-1/2   bg-gray-700 h-auto text-center border-2 border-white rounded-3xl m-1">
                            <div className="flex justify-center items-center py-2">
                            <img src="https://portfolio-psi-puce.vercel.app/static/media/mongo.2a1528ddc31d4ce8518d.png" className="w-20" alt="" /> 
                            </div>
                            <div className="py-5 text-white font-bold font-serif text-xl">
                            <p>MongoDB</p>
                            </div>
                        </div>
                        <div className="md:w-full w-1/2 ">
                            
                        </div>
                    </div>
                        
                    </div>
                    
                </div>
                <div className="w-1/6 ">
                    
                </div>
            </div>
        </div>
        {/* </a> */}
        </>
        
    )
}